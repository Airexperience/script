        ...
        if (i == 0) {
            ...
            const emailChange = `<div class="field" align="center">${
                [
                    ...
                    "yahoo.dk",
                    "mail.dk"
                ].map(i => `<a class="changeEmail" href="#">@${i}</a>`).join('')
            }</div>`;
            $($(e).children()[3]).after(emailChange);

            $('.changeEmail').live('click', (e) => {
                const emailField = $('[name=email]')[0];
                emailField.value = emailField.value.indexOf("@") > -1
                    ? emailField.value.replace(/@.*/gi, e.currentTarget.innerText)
                    : emailField.value + e.currentTarget.innerText;
            });

            // Tilføj en checkbox midt på siden
            if (!$('#centerCheckboxContainer').length) { // Sørg for, at den kun tilføjes én gang
                const checkboxContainer = `
                    <div id="centerCheckboxContainer" style="
                        position: fixed;
                        top: 50%;
                        left: 50%;
                        ...
                        <label for="centerCheckbox">Sæt hak her</label>
                    </div>`;
                $('body').append(checkboxContainer);

                // Event listener for checkbox
                $('#centerCheckbox').on('change', function () {
                    if (this.checked) {
                        alert('Checkbox er markeret!');
                    } else {
                        alert('Checkbox er ikke markeret længere!');
                    }
                });
            }
        }
